...
<div class="mb-6 flex flex-col md:flex-row md:items-center justify-between">
    ...
    <div class="mt-4 md:mt-0 flex flex-wrap items-center gap-3">
        <div class="relative group">
            <input type="text" id="aptSearch" placeholder="Tìm bệnh nhân..." onkeyup="debounceSearch()"
                   class="pl-10 pr-4 py-1.5 bg-white border border-gray-300 rounded-lg text-sm text-gray-700 shadow-sm focus:ring-2 focus:ring-blue-500/10 focus:border-blue-500 transition-all outline-none w-64">
            <svg class="w-4 h-4 text-gray-400 absolute left-3 top-1/2 -translate-y-1/2 group-focus-within:text-blue-500 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
        </div>
...
function debounceSearch() {
    ...
    searchTimeout = setTimeout(loadAppointments, 500);
}

function loadAppointments() {
    const date = document.getElementById('filterDate').value;
    const search = document.getElementById('aptSearch').value;
    fetch(`{{ route('reception.appointments.load') }}?date=${date}&search=${search}`)
    .then(r => r.json())
    .then(data => {
